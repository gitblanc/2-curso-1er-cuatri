...
public class Graph<T> {
	...
	//Los inaccesibles son los fuentes mas los aislados
	...
	public T averiguaCentro() {
		if(this.a == null) {
			floyd();
		}
		double[] vector = new double[size];
		for(int i = 0; i < this.size; i++) {
			vector[i] = maximoEnColumna(i);
		}
		int minimo = getMinimum(vector);
		if(minimo == -1) {
			return null;
		}
		return nodes[minimo];
	...
	private int getMinimum(double[] vector) {
	...
	private double maximoEnColumna(int fila) {
		double max = Double.NEGATIVE_INFINITY;
		for(int j = 0; j < this.size; j++) {
			if(getFloydA()[j][fila] > max) {
				max = getFloydA()[j][fila];
			}
		}
		return max;
	}
